...
import { Loader, LogIn, PanelLeft } from "lucide-react";
...
export default async function Home(props: {
  ...
  const experts = [
    ...
    .filter((expert) =>
      expert.title
        .toLowerCase()
        .includes(query?.toLowerCase() || expert.description.toLowerCase().includes(query?.toLowerCase()))
    );
  ...
  console.dir(filters[0]);
  console.log(
    `
  ====
  filter: `,
    { filter: filters[0].label, selected: filters[0].selected }
  );
